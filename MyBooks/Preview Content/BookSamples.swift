...
//  MyBooks
//
//  Created by Lucrezia Odrljin on 03.07.2024..
//

import Foundation

// nastavak za book model
extension Book {
    // static property, current date metoda dodaje dan od -7 dana do danas
    static let lastWeek = Calendar.current.date(byAdding: .day, value: -7, to: Date.now)!
    // static property, current date metoda dodaje mjesec od -1 do tekuceg mjeseca
    static let lastMonth = Calendar.current.date(byAdding: .month, value: -1, to: Date.now)!
    // static property array of books
